...
- **Cloudflare Turnstile** on `/signup` and `/login` (`lib/turnstile.ts`, `components/Turnstile.tsx`) is optional bot/abuse protection layered on top of the existing per-IP rate limiting. Without `NEXT_PUBLIC_TURNSTILE_SITE_KEY`/`TURNSTILE_SECRET_KEY` set, the widget doesn't render and no verification is required, on both the client and server side — see `.env.example` for setup.
