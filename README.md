...
- **Contributor dashboard** (`/dashboard`) — approved contributors submit and track their own city's articles.
...
- **Google reCAPTCHA v3** on `/signup`, `/login`, and `/admin/login` (`lib/recaptcha.ts`, `lib/recaptchaClient.ts`) is optional, invisible bot/abuse scoring layered on top of the existing per-IP rate limiting — no checkbox or widget, each submit silently fetches a fresh token and the server scores it. Without `NEXT_PUBLIC_RECAPTCHA_SITE_KEY`/`RECAPTCHA_SECRET_KEY` set, no token is requested and no verification is required, on both the client and server side — see `.env.example` for setup (the key must be created as type v3, not v2). If the script itself fails to load client-side (e.g. blocked by a DNS filter), `getRecaptchaToken()` resolves to `null` and the form submits anyway rather than getting hard-locked out; the server mirrors that by failing open on a missing token too, so rate limiting remains the real backstop.
...
- **Draft-first article editor** (`/dashboard/articles/new`, `/dashboard/articles/[id]/edit`): a new article starts as an unsubmitted `draft` and autosaves every ~2.5s. Only an explicit "Submit for Review" moves it to `pending` — nothing is ever silently submitted for the contributor.
...
- **Points/scoring** is user-side display only in this phase (`/dashboard/points`) — it reads whatever score/feedback the Admin Panel has already recorded. The Admin Panel's actual scoring UI is a separate, later phase.
...
- **Points & Scoring** (`/admin/points`): reads the same `articles.score`/`admin_feedback` columns the contributor's own `/dashboard/points` page reads — a score given here is visible to the contributor immediately on their next load, no separate sync step.
- **Media Library** (`/admin/media`): every uploaded image, searchable, with alt text/caption editing and delete. Delete checks live usage (city hero images, article covers, and a content-HTML substring scan) before deleting and asks for confirmation if the image is still referenced anywhere — this is a live scan on each delete attempt, not a persisted reverse-index, so it's always accurate but does one query pass across cities/articles each time.
- **SEO Settings** (`/admin/seo`) and **Site Settings** (`/admin/settings`) manage a single sitewide `settings` row — default meta description, default OG image, default robots directive, homepage intro override, featured cities, and a moderation note. These are deliberately scoped to fields that are actually wired into rendering (the homepage currently consults them); per-page SEO on individual articles and cities always takes precedence and lives on those records directly, not here.
- **Activity Log** (`/admin/activity`): every meaningful admin mutation (approvals, rejections, suspensions, scores, publish/unpublish, edits, deletes, city/category/media/settings changes) is recorded via `lib/activity.ts` and shown here with filters by admin, target type, date, and free text. Logging is best-effort — a logging failure never blocks the actual action.
