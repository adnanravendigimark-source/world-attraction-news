    ...
    <div className="space-y-4">
      ...
      {isOwnerAccount ? (
        <p className="rounded border border-ink-200 bg-ink-50 p-3 text-xs text-ink-600">
          This is the owner account, set via <code className="rounded bg-ink-200 px-1 py-0.5">ADMIN_EMAIL</code> /{" "}
          <code className="rounded bg-ink-200 px-1 py-0.5">ADMIN_PASSWORD</code> in your environment variables. It
          isn't a database account, so its password can't be changed here — update those environment variables and
          redeploy instead.
        </p>
      ) : (
