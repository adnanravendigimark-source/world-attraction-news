...
// A dependency-free, drag-to-crop tool — ported from the same pattern used
// in the Sagrada/Amsterdam admin editors (components/admin/CropModal.tsx
// there), restyled to this app's slate/red tokens. The full photo is always
// visible; the contributor drags a resizable box directly on top of it
// (locked to the target aspect ratio) to choose what to keep. "Save crop"
// renders exactly what's inside the box to a canvas and hands back a Blob
// ready to upload.
export default function InlineCropModal({
  src,
  aspectRatio,
  onCancel,
  onConfirm,
  ...
  const handleClass = "absolute h-4 w-4 rounded-full border-2 border-white bg-[#DC2626] shadow touch-none";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={(e) => e.stopPropagation()}>
      <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl">
        <h3 className="text-base font-semibold text-slate-900">Adjust crop</h3>
        <p className="mt-1 text-sm text-slate-500">Drag the box to reposition it, drag a corner handle to resize.</p>

        <div
          className="relative mx-auto mt-4 select-none overflow-hidden rounded-lg border border-slate-300 bg-slate-100"
