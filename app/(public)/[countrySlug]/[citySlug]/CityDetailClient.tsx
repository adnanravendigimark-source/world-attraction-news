...
export default function CityDetailClient({
  ...
  return (
    <div className="bg-white min-h-screen text-[#0B1527] pb-16">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="max-w-xl">
              ...
              <h1 className="font-serif text-3xl sm:text-4xl lg:text-[44px] font-black tracking-tight text-[#0B1527]">
                {city.name} Travel & News
              </h1>
              <p className="mt-2 text-xs sm:text-sm text-slate-600 leading-relaxed font-medium">
                {city.intro || `Verified on-the-ground reporting, visitor guides, and updates from our ${city.name} correspondents.`}
              </p>
            </div>
            ...
            <div className="relative rounded-xl border border-rose-100/80 bg-rose-50/40 p-4 sm:p-5 lg:w-[380px] overflow-hidden">
              ...
              <h3 className="font-sans text-xs font-black uppercase tracking-wider text-[#0B1527]">
                Get {city.name} Dispatches
              </h3>
              <p className="mt-1 text-[11px] text-slate-500 leading-normal">
                Receive the latest destination intelligence directly to your inbox.
              </p>
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0 scrollbar-none">
              {availableTags.map((tag) => (
                <button
                  ...
                  className={`rounded-full px-3.5 py-1.5 text-xs font-black uppercase tracking-wider transition-all whitespace-nowrap cursor-pointer ${
                    activeTag === tag
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative w-full sm:w-60">
                <input
                  ...
                  placeholder={`Search ${city.name} news...`}
                  className="w-full rounded-lg border border-slate-200 bg-white px-3 py-1.5 pr-8 text-xs text-slate-800 placeholder:text-slate-400 focus:border-[#DC2626] focus:outline-none"
                />
                {searchKeyword ? (
                  <button
                    type="button"
                  ...
                  <svg className="absolute right-2.5 top-2 h-3.5 w-3.5 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <select
                ...
                className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 focus:border-[#DC2626] focus:outline-none cursor-pointer shrink-0"
              >
      ...
      {/* =========================================
          3. MAIN CONTENT: ARTICLES LIST & SIDEBAR
      ========================================= */}
      <section className="py-8 sm:py-10">
        <Container>
          <div className="grid gap-8 lg:grid-cols-12 items-start">
            {/* Left Articles Stream (68% / 8 cols) */}
            <div className="lg:col-span-8 flex flex-col gap-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                ...
                <span className="text-xs text-slate-400">
                  Showing {filteredArticles.length === 0 ? 0 : (page - 1) * PAGE_SIZE + 1}–
                  {Math.min(page * PAGE_SIZE, filteredArticles.length)} of {filteredArticles.length}
                </span>
              </div>
              ...
              {pagedArticles.length === 0 ? (
                <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
                  <EmptyState
                    title={`No articles found for ${city.name}`}
                    description={
                      searchKeyword
                        ? `No dispatches match "${searchKeyword}". Try clearing your search keyword.`
                        : `Our ${city.name} correspondents are actively working on new stories. Check back shortly.`
                    }
                  />
                  {searchKeyword && (
                    <button
                      type="button"
                      onClick={() => setSearchKeyword("")}
                      className="mt-4 inline-flex rounded-lg bg-[#DC2626] px-4 py-2 text-xs font-bold text-white hover:bg-[#B91C1C] cursor-pointer"
                    >
                      Clear Search
                    </button>
                  )}
            ...
            <div className="lg:col-span-4 flex flex-col gap-6">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6 shadow-sm">
                ...
                <div className="p-3.5 bg-slate-50 rounded-xl mb-4 text-center">
                  <p className="text-2xl font-black text-[#0B1527]">{articles.length}</p>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mt-0.5">Published Dispatches</p>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                  ...
                  >
                    <span>Explore {city.country} Hub</span>
              ...
              <div className="relative overflow-hidden rounded-2xl bg-[#0B1527] text-white p-6 shadow-md flex flex-col gap-3">
                ...
                <p className="text-xs text-slate-300 leading-relaxed max-w-[260px]">
                  Submit your travel dispatches, photo reports, or scoop tips for {city.name}.
                </p>
                <Link
                  href="/write-for-us"
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-500 leading-relaxed max-w-lg">
                  Get destination intelligence and updates delivered to your inbox.
                </p>
              </div>
            </div>
