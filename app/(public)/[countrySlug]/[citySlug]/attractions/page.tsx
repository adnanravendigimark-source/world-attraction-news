...
import { cityPath, countryPath, attractionsPath, attractionPath } from "@/lib/destinations";
import { SITE_NAME } from "@/lib/site";

// Pure read, no searchParams — real ISR. Admin attraction create/edit/delete
// calls revalidatePath(attractionsPath(countrySlug, citySlug)),
// i.e. revalidatePath(`/${countrySlug}/${citySlug}/attractions`).
//
...
export async function generateMetadata({
  ...
  return buildMetadata({
    ...
    path: attractionsPath(city.countrySlug, city.slug),
  });
}
...
export default async function CityAttractionsPage({
  ...
  const breadcrumbs = [
    ...
    { name: "Attractions", path: attractionsPath(city.countrySlug, city.slug) },
  ];
      ...
      <div className="border-b border-slate-100 bg-white pt-5 pb-8">
        <Container>
          ...
          <nav className="flex items-center gap-1.5 text-xs font-semibold text-slate-400 mb-4">
            ...
            <span>&gt;</span>
            <Link href={cityPath(city.countrySlug, city.slug)} className="hover:text-slate-900 transition-colors">
              {city.name}
            </Link>
            <span>&gt;</span>
        ...
        <Container>
          ...
          {attractions.length === 0 ? (
            <EmptyState
              ...
              actionHref={cityPath(city.countrySlug, city.slug)}
            />
          ) : (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {attractions.map((a) => {
                ...
                return (
                  <Link
                    key={a.id}
                    href={attractionPath(city.countrySlug, city.slug, a.slug)}
