...
import { cityPath, countryPath, attractionPath } from "@/lib/destinations";

// Pure read — real ISR. Admin attraction edits call
// revalidatePath(attractionPath(countrySlug, citySlug, attractionSlug)),
// i.e. revalidatePath(`/${countrySlug}/${citySlug}/attractions/${attractionSlug}`).
//
...
export async function generateMetadata({
  ...
  if (!attraction || attraction.countrySlug !== params.countrySlug) return {};
  return buildMetadata({
    ...
    path: attractionPath(attraction.countrySlug, attraction.citySlug, attraction.slug),
...
export default async function AttractionPage({
  ...
  if (attraction.countrySlug !== params.countrySlug) {
    permanentRedirect(attractionPath(attraction.countrySlug, attraction.citySlug, attraction.slug));
  }
  ...
  const breadcrumbs = [
    ...
    { name: attraction.cityName, path: cityPath(attraction.countrySlug, attraction.citySlug) },
    { name: attraction.name, path: attractionPath(attraction.countrySlug, attraction.citySlug, attraction.slug) },
  ];
      ...
      <section className="relative border-b-2 border-ink-950 bg-ink-950">
        ...
        <Container className={attraction.heroImage ? "absolute inset-x-0 bottom-0 pb-8" : "py-8"}>
          ...
          <div className="flex flex-wrap items-center gap-2 mb-2">
            <Link
              href={cityPath(attraction.countrySlug, attraction.citySlug)}
        ...
        {articles.length === 0 ? (
          <div className="mt-8">
            <EmptyState
              ...
              actionHref={cityPath(attraction.countrySlug, attraction.citySlug)}
            />
          </div>
        ) : (
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: jsonLdScript([
            ...
            itemListJsonLd(
              articles.map((a) => ({ name: a.title, path: `${cityPath(a.countrySlug, a.citySlug)}/${a.slug}` }))
            ),
          ]),
        }}
      />
